...
import { QuizQuestion, getControlLevelES, formatCurrencyES, incomeRangesES, calculateEstimatedValueES } from '@/lib/quiz-data-es';
import { useSound } from '@/hooks/use-sound';

// Componente de animación de monedas volando
...
function AnimatedCounter({ value, prefix = '' }: { value: number; prefix?: string }) {
  ...
  return <span>{prefix}{formatCurrencyES(displayValue)}</span>;
}

export default function QuestionScreenES({
  question,
  currentQuestion,
  totalQuestions,
  totalValue,
  ...
  const controlLevel = getControlLevelES(displayValue, incomeIndex, currentQuestion);
  ...
  const handleSelect = (option: typeof question.options[0], index: number, event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (!question.isIncomeQuestion) {
      ...
      const newValue = calculateEstimatedValueES(incomeIndex, newMultipliers);
      setTimeout(() => {
      ...
      <div className="max-w-lg w-full mx-auto mb-8">
        ...
        <div className="mb-4">
          <div className="flex justify-between text-sm text-slate-500 mb-2">
            <span className="font-medium">Pregunta {currentQuestion} de {totalQuestions}</span>
          ...
          >
            ...
            <div className="bg-white rounded-xl p-3 border border-slate-200 shadow-sm">
              <div className="flex items-center gap-2 text-slate-500 text-xs mb-1">
                ...
                <span>Nivel de Fuga</span>
              </div>
              <div className={`font-bold text-lg ${
                controlLevel?.level === 'Bajo' ? 'text-green-600' :
