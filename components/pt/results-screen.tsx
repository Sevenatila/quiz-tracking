...
export default function ResultsScreenPT({ totalValue, incomeIndex, onContinue }: ResultsScreenProps) {
  ...
  const stats = [
    ...
    {
      icon: BarChart3,
      label: 'Nivel de Vazamento',
      textValue: controlLevel?.level ?? 'N/A',
      color: controlLevel?.level === 'M\u00e9dio' ? 'text-yellow-600' :
             controlLevel?.level === 'Alto' ? 'text-red-600' : 'text-green-600',
      bgColor: controlLevel?.level === 'M\u00e9dio' ? 'bg-yellow-50' :
               controlLevel?.level === 'Alto' ? 'bg-red-50' : 'bg-green-50',
      borderColor: controlLevel?.level === 'M\u00e9dio' ? 'border-yellow-100' :
                   controlLevel?.level === 'Alto' ? 'border-red-100' : 'border-green-100'
    }
  ];
  ...
  return (
    ...
    >
      <div className="max-w-lg w-full">
        ...
        >
          <p className="text-slate-600 text-xs mb-1">Estimativa de perda mensal</p>
        ...
        >
          <p className="text-slate-500 text-sm mb-3">
            Seu painel financeiro personalizado est&aacute; pronto...
          </p>
          <motion.button
          ...
          >
            Ver meu diagn&oacute;stico completo
