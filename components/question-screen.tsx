...
// Componente de animação de moedas voando
function FlyingMoney({ 
  ...
  const coins = [0, 1, 2, 3, 4]; // 5 moedas
...
export default function QuestionScreen({
  ...
  // Hook de som e vibração
  const { playClickSound, playCoinSound } = useSound();
  
  // Reset COMPLETO quando mudar de pergunta - usando question.id como trigger principal
  useEffect(() => {
  ...
  }, [question.id, totalValue]); // Dependência do ID da pergunta para garantir reset
  
  // Atualizar displayValue quando totalValue mudar (sem resetar seleção)
  useEffect(() => {
    if (!isTransitioning) {
      setDisplayValue(totalValue);
    }
  ...
  // Só mostrar stats quando já houver valor calculado (após responder a 2ª pergunta)
  ...
  const handleSelect = (option: typeof question.options[0], index: number, event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    // Animação de moedas voando (só se não for pergunta de renda e se temos o target)
    if (!question.isIncomeQuestion && moneyTargetRef.current) {
      // Som de moeda quando as moedas voam
    ...
    // Calcula o novo valor imediatamente para mostrar ao usuário
    ...
    setTimeout(() => {
      ...
      // NÃO resetar estados aqui - o useEffect fará isso quando currentQuestion mudar
    }, 600);
  };
  ...
  return (
    ...
    >
      ...
      <div className="max-w-lg w-full mx-auto mb-8">
        ...
        {/* Stats Cards - só aparecem após a primeira pergunta */}
        {showStats && (
          <motion.div 
          ...
          >
            <div ref={moneyTargetRef} className="bg-white rounded-xl p-3 border border-slate-200 shadow-sm relative overflow-hidden">
              <div className="flex items-center gap-2 text-slate-500 text-xs mb-1">
                ...
                <span>Gastos Invisíveis</span>
              </div>
              <div className="text-red-600 font-bold text-lg">
                <AnimatedCounter value={displayValue} prefix="-" />
              </div>
              {/* Efeito de brilho quando recebe moedas */}
              {flyingMoney && (
                <motion.div
                  initial={{ opacity: 0 }}
            ...
            <div className="bg-white rounded-xl p-3 border border-slate-200 shadow-sm">
              <div className="flex items-center gap-2 text-slate-500 text-xs mb-1">
                ...
                <span>Nível de Vazamento</span>
              </div>
              <div className={`font-bold text-lg ${
                controlLevel?.level === 'Baixo' ? 'text-green-600' :
